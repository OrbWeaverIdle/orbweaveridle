...
(function () {
  ...
  cardModalScrim.addEventListener('click', (e) => {
    if (e.target === cardModalScrim) {
      closeModal(cardModalScrim);
      el('card-modal-win').classList.remove('mule-modal-win');
    }
  });
  el('card-modal-close').addEventListener('click', () => {
    closeModal(cardModalScrim);
    el('card-modal-win').classList.remove('mule-modal-win');
  });
