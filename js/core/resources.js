...
(function () {
  ...
  const registry = {}; // id -> { current, cap, displayType, valEl, rateEl }
  ...
  function register(id, { name, mount, current, cap, displayType = 'decimal' }) {
    const wrap = document.createElement('div');
    const row = document.createElement('div');
    row.className = 'resource-row';
    row.innerHTML = `<span class="res-name">${name}</span><span class="res-val"></span>`;

    // Sub-rate label is a sibling below the row, not a child of it,
    // so it drops to its own line instead of sharing the flex row.
    ...
    registry[id] = { current, cap, displayType, valEl: row.querySelector('.res-val'), rateEl };
    render(id);
    return wrap;
  }
  ...
  // Shows/hides the sub-rate label under a resource row.
  // rateText: e.g. '+2.6/s' or '-3/s'. Pass null to hide.
  ...
  window.OrbWeaver.Resources = { register, exists, get, getCap, isAtCap, all, add, spend, setCap, setSubRate };
})();
