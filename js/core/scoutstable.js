...
(function () {
  ...
  function createScoutStable(config) {
    ...
    // Local to this Scout Stable instance — Camp's and Mountains' each
    // own their own, so a future tool can speed up one without the
    // other. f=0 (not the old formula's hardcoded 2) so 0 workers loads
    // nothing, matching every other card's "workers ARE the clock" rule.
    let m = 1, p = 1, f = 0;
    ...
    const mechanic = {
      ...
      getWorkerDesc() {
        ...
        return assigned === 0 ? '' : `Loading ${window.OrbWeaver.Upgrades.formatRate(window.OrbWeaver.Upgrades.workerRate(assigned, m, p, f))}/s`;
      },
      ...
      tick(goldAvailable, tickRate) {
        ...
        const assigned = pool().getAssigned(ID);
        const perTick = tickRate * window.OrbWeaver.Upgrades.workerRate(assigned, m, p, f);
