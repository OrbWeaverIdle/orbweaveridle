...
(function () {
  ...
  function create(table, renderEffect, collapsible) {
    ...
    return { current, start, cancel, collectProduced, advanceTimer, isBuilding, isCollecting,
             getCardProgressPct, renderModalHTML, toggleCollapse, isCollapsed,
  ...
  function createChoiceTrack(items, onComplete) {
    ...
    return { unlock, revealWithGold, start, cancel, isBuilding, activeItem, getRemaining, getProgressPct, advanceTimer, renderModalRows, patchModalRows };
  }
  ...
  function createSellTrack(defs, config) {
    ...
    function startSale(id) {
      ...
      r.sale = { elapsed: 0, gold: goldFor(r), amt: a }; // duration recomputed live every tick — see tick()
    }
    ...
    // Duration is NOT frozen at sale start — it's re-derived from the
    // card's current worker count every tick, so assigning/unassigning
    // workers mid-sale speeds up or slows down the remaining time live.
    function tick(tickRate) {
      const total = projectedTime();
      res.forEach((r) => {
        if (!r.sale) return;
        r.sale.elapsed += tickRate;
        if (r.sale.elapsed < total) return;
    ...
    function getSoonestRemaining() {
      let m = null;
      const total = projectedTime();
      res.forEach((r) => { if (r.sale) { const rem = Math.max(0, total - r.sale.elapsed); if (m == null || rem < m) m = rem; } });
      return m;
    }
    function getBuildBarPct() {
      let bestRem = null;
      const total = projectedTime();
      res.forEach((r) => { if (r.sale) { const rem = total - r.sale.elapsed; if (bestRem == null || rem < bestRem) bestRem = rem; } });
      return bestRem == null ? 0 : Math.min(100, ((total - bestRem) / total) * 100);
    }
    ...
    function patchDOM() {
      ...
      if (timeEl) timeEl.textContent = `— ${total.toFixed(1)}s`;
      res.filter((r) => r.unlocked).forEach((r) => {
        const statusEl = document.querySelector(`#card-modal-body [data-res="${r.id}"] .sell-status`);
        if (statusEl) statusEl.textContent = r.sale ? `Selling — ${Math.ceil(Math.max(0, total - r.sale.elapsed))}s` : '';
        const barEl = document.querySelector(`#card-modal-body [data-res="${r.id}"] .sell-progress-bar`);
        if (barEl) barEl.style.width = (r.sale ? Math.min(100, (r.sale.elapsed / total) * 100) : 0).toFixed(1) + '%';
        if (!r.sale) {
    ...
    return {
      ...
      getSoonestRemaining, getBuildBarPct, projectedTime, renderCards, patchDOM
    };
  }
