...
(function () {
  ...
  function makeStepperSet(mechanic, isModal) {
    ...
    addHoldBehavior(plus, () => mechanic._mountainsWorkerCard ? mechanic._assign() : window.OrbWeaver.Workers.assign(mechanic.id));
    addHoldBehavior(minus, () => mechanic._mountainsWorkerCard ? mechanic._unassign() : window.OrbWeaver.Workers.unassign(mechanic.id));
    return wrap;
  }
  ...
  function refreshModalSteppers(mechanic) {
    ...
    if (!ms) return;
    const assigned = mechanic._mountainsWorkerCard ? mechanic._getAssigned() : window.OrbWeaver.Workers.getAssigned(mechanic.id);
    const idleCount = mechanic._mountainsWorkerCard
      ? (window.OrbWeaver.Mountains ? window.OrbWeaver.Mountains.getIdle() : 0)
      : window.OrbWeaver.Workers.getIdleCount();
    ms.querySelector('.card-stepper-val').textContent = assigned;
    ms.querySelector('.stepper-minus').disabled = assigned === 0;
    ms.querySelector('.stepper-plus').disabled = idleCount === 0;
    if (mechanic.getWorkerDesc) {
  ...
  function refresh(mechanic) {
    ...
    mechanic.els.cardStepperPlus.disabled = idleCount === 0;
    refreshModalSteppers(mechanic);
