/* ============================================================
   ...
   A mechanic may optionally define:
     ...
     mechanic.upgradeBarTiered — colors the card-face top bar with
       var(--tier) instead of the default green (style.css's
       .card-upgrade-bar.tiered)
     mechanic.upgradeTrackKey / mechanic.buildTrackKey — data-track
       values used to find the right bar inside a multi-track modal
     mechanic.getExtraBars()   — optional list of { id, pct } for any
       further modal-only bars beyond the fixed top/bottom pair (e.g. a
       branching chain where more than one sub-task can be active at
       once). Eased by the same requestAnimationFrame loop as the top/
       bottom bars; each one's rendered element needs
       data-extra-track="<id>". Bars are added/removed automatically as
       ids appear/disappear from the list.
...
(function () {
  ...
  function addHoldBehavior(btn, action) {
    ...
    // A held button can, in principle, be replaced mid-hold by a genuine
    // structural rebuild (a section it belongs to disappearing). Without
    // this check the old element's timers would keep firing forever,
    // since nothing else stops them once they can no longer receive the
    // pointerup that would. isConnected is false the instant a node
    // leaves the document, so this is a safe no-op for every ordinary
    // stepper, which is never replaced mid-hold.
    function fire() {
      if (!btn.isConnected) { stop(); return; }
      if (!btn.disabled) action(); else stop();
    }
  ...
  function startSmoothing(mechanic) {
    ...
    // Extra modal-only bars beyond the fixed top/bottom pair, keyed by
    // their own id so any number can appear/disappear over time without
    // disturbing each other's eased state (see mechanic.getExtraBars()).
    const extraState = new Map();
    ...
    function frame(now) {
      ...
      // Extra bars: same easing math as above, one per currently-active
      // id, modal-only (no card-face equivalent exists for these).
      if (mechanic.getExtraBars && openModalMechanicId === mechanic.id) {
        const seen = new Set();
        mechanic.getExtraBars().forEach((bar) => {
          seen.add(bar.id);
          let es = extraState.get(bar.id);
          if (!es) { es = { shown: bar.pct, from: bar.pct, to: bar.pct, start: now, dur: 0 }; extraState.set(bar.id, es); }
          if (bar.pct !== es.to) { es.from = es.shown; es.to = bar.pct; es.start = now; es.dur = window.OrbWeaver.Loop.getIntervalMs(); }
          const t = es.dur > 0 ? Math.min(1, (now - es.start) / es.dur) : 1;
          es.shown = es.from + (es.to - es.from) * t;
          const el = document.querySelector(`#card-modal-body [data-extra-track="${bar.id}"]`);
          if (el) el.style.width = es.shown + '%';
        });
        // Drop bars no longer reported so a reused id starts fresh, not
        // mid-way through a stale animation.
        Array.from(extraState.keys()).forEach((id) => { if (!seen.has(id)) extraState.delete(id); });
      }
  ...
  function build(mechanic) {
    ...
    // Optional: mechanic.upgradeBarTiered colors the top bar with the
    // surrounding section's --tier (e.g. Mountains' blue) instead of the
    // default green — reusable by any future card via style.css's
    // .card-upgrade-bar.tiered rule, no per-card selector needed.
    upgradeBar.className = 'card-upgrade-bar' + (mechanic.upgradeBarTiered ? ' tiered' : '');
