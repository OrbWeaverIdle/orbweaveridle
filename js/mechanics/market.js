/* ============================================================
   ...
   1) Self-upgrade ladder (top bar, collapsible) — the SAME collapsible
      ...
      rate (sellSecondsPerWorker) and applies a structured `effect` that
   ...
   Sell timing now uses the SAME rate-multiplier mechanism as any
   self-upgrade ladder (js/core/upgrades.js: tickRate + assigned×spw×
   tickRate, accelerating an in-progress countdown rather than shrinking
   a target time) — sellSecondsPerWorker is Market's own per-tier value
   for that rate, raised by SELF_TABLE tiers exactly like Builder's
   Bench's secondsPerWorker raises its own build speed. floorTime (8s)
   is layered on top as a real-time-elapsed hard minimum unique to
   Market — see the sell-track's own header comment in upgrades.js.
   ============================================================ */
(function () {
  'use strict';
  ...
  let sellSecondsPerWorker = 0.2;    // rebalanced down from 0.5 — see market.js's header comment
  let opened = false;
  let tipVisible = true;
  ...
  function onSelfComplete(row) {
    cardName = row.name;
    sellSecondsPerWorker = row.gainPerWorker;
    const e = row.effect || {};
  ...
  const mechanic = {
    ...
    getWorkerDesc: () => {
      ...
      const saved = Math.round(SELL_CONFIG.baseTime - SELL_CONFIG.baseTime / (1 + a * sellSecondsPerWorker));
      return a <= 1 ? `1 worker reducing -${saved}s` : `${a} workers reducing -${saved}s`;
    },
