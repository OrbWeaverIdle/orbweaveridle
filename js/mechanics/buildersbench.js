/* ============================================================
   ...
   reads "{n} to build" (Construction items still available) or, once
   none remain, "Next upgrade – {cost} – {time}s" (self-ladder's next
   tier); building reads "Building X – Ys".
   ============================================================ */
(function () {
  'use strict';
  ...
  const CONSTRUCTION_ITEMS = [
    { id: 'campsite', name: 'Campsite', buildTime: 2, costRaw: '15 wood', startAvailable: true },
  ...
  function onConstructionComplete(id) {
    if (id === 'stonepit') {
      ...
      window.OrbWeaver.Cards.reveal(window.OrbWeaver.Mechanics.get('campsite'));
    } else if (id === 'scoutspen') {
      window.OrbWeaver.Cards.reveal(window.OrbWeaver.Mechanics.get('scoutspen'));
    }
    // gamblersmat: inert for now.
  }
  ...
  const mechanic = {
    ...
    getStatText() {
      ...
      if (remaining > 0) {
        const idle = `${remaining} to build`;
        return collecting ? `${idle} – Collecting` : idle;
      }
      const nextTier = selfTrack.current();
      const idle = nextTier ? `Next upgrade – ${nextTier.costRaw} – ${nextTier.buildTime}s` : '';
      return collecting ? `${idle} – Collecting`.trim() : idle;
    },
