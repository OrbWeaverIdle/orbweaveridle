...
(function () {
  ...
  /* ---- Wheat Field build ----
     ...
     drain: 300 wood deducted all at once when Build is clicked, a flat
     worker-driven timer, Cancel refunds the full amount. Unlocked once
  ...
  const WHEAT_WOOD_COST = 300, WHEAT_BUILD_TIME = 10;
  ...
  /* ---- Gold Panning build ----
     Same hand-rolled pattern as Wheat Field: upfront cost on Build,
     flat worker-driven timer, Cancel refunds in full. Unlocked when
     the 'surroundedmountains' travel path completes. Reveals the
     Gold Panning card on completion. */
  const GOLDPAN_WOOD_COST = 100, GOLDPAN_STONE_COST = 100, GOLDPAN_BUILD_TIME = 20;
  ...
  // Wheat Field's build option — fully hand-rolled hs-card matching
  // Prepare Stone Quarry's exact idle/building structure (stepper lives
  // inside the card, not appended as a sibling), with a simple upfront-
  // cost + flat-timer mechanic underneath (not Quarry's gradual drain).
  function wheatFieldSection() {
    ...
    // idle
    const wood = window.OrbWeaver.Resources.get(WOOD_ID);
    const affordable = wood >= WHEAT_WOOD_COST;
    return `<div class="hs-card${affordable ? '' : ' hs-card-dim'}" data-wheat-card>
      <div class="hs-head">
        <span class="hs-title">Wheat Field</span>
        <span class="upgrade-costtime wheat-cost-label">${Math.round(wood)}/${WHEAT_WOOD_COST} Wood – ${WHEAT_BUILD_TIME}s</span>
      </div>
      <div class="hs-body">
        <div class="hs-desc-build-row">
          <span class="upgrade-desc${affordable ? ' upgrade-desc-affordable' : ''}">TBD</span>
          <button class="action-btn hs-btn-right" data-mtn-action="start-wheat" ${affordable ? '' : 'disabled'}>Build</button>
        </div>
      </div></div>`;
  }

  // Gold Panning build — same pattern as Wheat Field.
  function goldPanSection() {
    ...
    const wood = window.OrbWeaver.Resources.get(WOOD_ID);
    const stone = window.OrbWeaver.Resources.get(STONE_ID);
    const affordable = wood >= GOLDPAN_WOOD_COST && stone >= GOLDPAN_STONE_COST;
    return `<div class="hs-card${affordable ? '' : ' hs-card-dim'}" data-goldpan-card>
      <div class="hs-head">
        <span class="hs-title">Gold Panning</span>
        <span class="upgrade-costtime goldpan-cost-label">${Math.round(wood)}/${GOLDPAN_WOOD_COST} Wood, ${Math.round(stone)}/${GOLDPAN_STONE_COST} Stone – ${GOLDPAN_BUILD_TIME}s</span>
      </div>
      <div class="hs-body">
        <div class="hs-desc-build-row">
          <span class="upgrade-desc${affordable ? ' upgrade-desc-affordable' : ''}">Pan the river for gold.</span>
          <button class="action-btn hs-btn-right" data-mtn-action="start-goldpan" ${affordable ? '' : 'disabled'}>Build</button>
        </div>
      </div></div>`;
  }
  ...
  const mechanic = {
    ...
    patchLiveTrack() {
      ...
      if (wheatFieldUnlocked()) {
        ...
        } else if (wheatPhase === 'idle') {
          const card = document.querySelector('#card-modal-body [data-wheat-card]');
          if (card) {
            const wood = window.OrbWeaver.Resources.get(WOOD_ID);
            const affordable = wood >= WHEAT_WOOD_COST;
            card.classList.toggle('hs-card-dim', !affordable);
            const costLbl = card.querySelector('.wheat-cost-label');
            if (costLbl) costLbl.textContent = `${Math.round(wood)}/${WHEAT_WOOD_COST} Wood – ${WHEAT_BUILD_TIME}s`;
            const desc = card.querySelector('.upgrade-desc');
            if (desc) desc.classList.toggle('upgrade-desc-affordable', affordable);
            const btn = card.querySelector('[data-mtn-action="start-wheat"]');
      ...
      if (goldPanUnlocked()) {
        ...
        } else if (goldPanPhase === 'idle') {
          const card = document.querySelector('#card-modal-body [data-goldpan-card]');
          if (card) {
            const wood = window.OrbWeaver.Resources.get(WOOD_ID);
            const stone = window.OrbWeaver.Resources.get(STONE_ID);
            const affordable = wood >= GOLDPAN_WOOD_COST && stone >= GOLDPAN_STONE_COST;
            card.classList.toggle('hs-card-dim', !affordable);
            const costLbl = card.querySelector('.goldpan-cost-label');
            if (costLbl) costLbl.textContent = `${Math.round(wood)}/${GOLDPAN_WOOD_COST} Wood, ${Math.round(stone)}/${GOLDPAN_STONE_COST} Stone – ${GOLDPAN_BUILD_TIME}s`;
            const desc = card.querySelector('.upgrade-desc');
            if (desc) desc.classList.toggle('upgrade-desc-affordable', affordable);
            const btn = card.querySelector('[data-mtn-action="start-goldpan"]');
            if (btn) btn.disabled = !affordable;
          }
        }
      }
  ...
  document.addEventListener('click', (e) => {
    ...
    if (btn.dataset.mtnAction === 'start-quarry' && quarryPhase === 'idle') {
      ...
      if (window.OrbWeaver.Resources.get(WOOD_ID) >= WHEAT_WOOD_COST) {
        window.OrbWeaver.Resources.spend(WOOD_ID, WHEAT_WOOD_COST);
        wheatPhase = 'building';
        wheatTimeRemaining = WHEAT_BUILD_TIME;
        window.OrbWeaver.Cards.refreshOpenModal(ID);
      }
    } else if (btn.dataset.mtnAction === 'cancel-wheat' && wheatPhase === 'building') {
      window.OrbWeaver.Resources.add(WOOD_ID, WHEAT_WOOD_COST); // full refund — cost was deducted upfront, not drained
      wheatPhase = 'idle';
      wheatTimeRemaining = WHEAT_BUILD_TIME;
      wheatWorkers = 0;
      window.OrbWeaver.Cards.refreshOpenModal(ID);
      ...
      const wood = window.OrbWeaver.Resources.get(WOOD_ID);
      const stone = window.OrbWeaver.Resources.get(STONE_ID);
      if (wood >= GOLDPAN_WOOD_COST && stone >= GOLDPAN_STONE_COST) {
        window.OrbWeaver.Resources.spend(WOOD_ID, GOLDPAN_WOOD_COST);
        window.OrbWeaver.Resources.spend(STONE_ID, GOLDPAN_STONE_COST);
      ...
      window.OrbWeaver.Resources.add(WOOD_ID, GOLDPAN_WOOD_COST);
      window.OrbWeaver.Resources.add(STONE_ID, GOLDPAN_STONE_COST);
      goldPanPhase = 'idle';
      goldPanTimeRemaining = GOLDPAN_BUILD_TIME;
      goldPanWorkers = 0;
      window.OrbWeaver.Cards.refreshOpenModal(ID);
