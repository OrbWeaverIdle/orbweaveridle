...
(function () {
  ...
  // Each track owns its own — a future tool could raise Research
  // Progress's pace without touching the Papers→Journals conversion.
  let topicsM = 1, topicsP = 1, topicsF = 0;
  let convM = 1, convP = 1, convF = 0;
  ...
  const conversionTrack = window.OrbWeaver.Upgrades.createConversionTrack(CONVERSION_RECIPES, {
    ...
    getM: () => convM, getP: () => convP, getF: () => convF
  });
  ...
  const mechanic = {
    ...
    getWorkerDesc: () => {
      ...
      const rate = window.OrbWeaver.Upgrades;
      return `Researching ${rate.formatRate(rate.workerRate(assigned, topicsM, topicsP, topicsF))}/s · Converting ${rate.formatRate(rate.workerRate(assigned, convM, convP, convF))}/s`;
    },
    ...
    tick(goldAvailable, tickRate) {
      ...
      const assigned = window.OrbWeaver.Workers.getAssigned(ID);
      if (goldAvailable) {
        if (topicsTrack.isBuilding()) topicsTrack.advanceTimer(tickRate, assigned, topicsM, topicsP, topicsF);
