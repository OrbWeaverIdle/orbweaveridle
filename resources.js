...
(function () {
  ...
  const SELF_TABLE = [
    ...
    { name: 'Market Centre',   gainPerWorker: 1,   buildTime: 220,  costRaw: '220 wood, 100 stone',       desc: 'Sell Simple Parts',
      effect: { unlock: ['simpleparts'] } },
    { name: 'Market Square',   gainPerWorker: 1,   buildTime: 420,  costRaw: '550 wood, 650 stone',       desc: 'Auto-sell Simple Parts; larger Wood/Stone bundles',
      effect: { auto: ['simpleparts'], bundles: { wood: [200, 500], stone: [200, 500] } } },
    { name: 'Market District', gainPerWorker: 1,   buildTime: 600,  costRaw: '700 wood, 650 stone',       desc: 'Sell 2 resources at once; larger Simple Parts bundle',
      effect: { concurrent: 2, bundles: { simpleparts: [200] } } },
    { name: 'Trading Post',    gainPerWorker: 2,   buildTime: 1200, costRaw: '900 wood, 900 stone',       desc: 'Sell Simple Tools',
  ...
  const mechanic = {
    ...
    getWorkerDesc: () => `Sell time ${Math.round(sellTrack.projectedTime())}s`,
    renderModalHTML() {
      ...
      const tip = tipVisible ? `<div class="market-tip">${TIP}</div>` : '';
      return `<div class="market-modal">` +
        tip +
        `<div class="market-note">Active Workers cost no gold during sales</div>` +
        `<div class="modal-subsection-label has-arrow"><span>Upgrades</span>${arrow}</div>${selfTrack.renderModalHTML(ID, {}, 'self')}` +
        `<div class="modal-subsection-label divider-top">Sell</div>${sellTrack.renderCards(ID, 'sell')}` +
      `</div>`;
    },
